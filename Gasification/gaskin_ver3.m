...
for i = 1:dim(2)
%     data(data(:,i)>=1,i) = 0.9999;
    y(:,i) = smooth(data(:,i),rate(:,i)*scale,0.05,'rloess','Robust'); % smoothing
    [xx, ind] = unique(data(:,i));
    sample(:,i) = interp1(xx,y(ind,i),alpha,'spline');           % sampling
...
%     sample(:,i) = f{i}(alpha);
%     
    f{i} = fminunc(@(x) sum((model_RPM(alpha,x(1),x(2))-sample(:,i)).^2), [2; 3]);
    ...
    plot(alpha, scale*model_integrated(alpha,k(i),m(i)));
    hold off
